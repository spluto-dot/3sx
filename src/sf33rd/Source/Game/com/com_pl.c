...
void Main_Program(PLW* wk) {
    ...
    Attack_Flag[wk->wu.id] = ps.plw[wk->wu.id ^ 1].caution_flag;
    Check_At_Count(wk);
...
void Damage_2nd(PLW* wk) {
    ...
    if (Receive_Flag[wk->wu.id] != 0 && ps.plw[wk->wu.id].uot_cd_ok_flag != 0) {
        Lever_Buff[wk->wu.id] = 2;
    }
        ...
        if (wk->wu.cg_type == 12) {
            ...
            if (CP_Index[wk->wu.id][0] == 0xFF) {
                ...
                if (ps.plw[wk->wu.id].sa->ok &&
                    Arts_Super_Name_Data[wk->player_number][ps.plw[wk->wu.id].sa->kind_of_arts] != -1) {
                    CP_Index[wk->wu.id][0] =
                        Arts_Super_Name_Data[wk->player_number][ps.plw[wk->wu.id].sa->kind_of_arts];
                }
            }
        }
...
s32 Command_Attack_SP(PLW* wk, s8 Pl_Number, s16 Tech_Number, s16 Power_Level) {
    ...
    default:
        ...
        if (wk->wu.routine_no[1] == 0 && ps.plw[wk->wu.id].caution_flag == 0) {
            return 1;
        }
    }
