...
s32 home_visitor_check(PLW* wk) {
    ...
    if (wk->wu.id) {
        pl_num = ps.plw[0].player_number;
    } else {
        pl_num = ps.plw[1].player_number;
    }
...
void appear_data_init_set(PLW* wk) {
    ...
    if (bg_w.area) {
        ...
        ap_work = app_type_tbl2[wk->player_number][ps.plw[id_work].player_number][bg_w.stage];
    } else {
        ap_work = app_type_tbl[wk->player_number][ps.plw[id_work].player_number][bg_w.stage];
    }
...
void Appear_07000(PLW* wk) {
    ...
    case 0:
        ...
        if (ps.plw[wk->wu.id ^ 1].player_number == 12 && bg_w.stage == 12 && bg_w.area == 0) {
...
s32 sean_appear_check(PLW* wk, s16 id) {
    if (ps.plw[id].player_number == 12 && bg_w.stage == 12) {
        return 1;
    }
...
s16 gill_appear_check() {
    ...
    if (ps.plw[0].player_number == 0) {
        id_w = 1;
    }

    if (ps.plw[1].player_number == 0) {
        id_w = 0;
    }
...
void Appear_18000(PLW* wk) {
    ...
    case 0:
        ...
        if (ps.plw[0].player_number == 8 && ps.plw[1].player_number == 8) {
            Appear_free[wk->wu.id] = 0;
...
void Appear_28000(PLW* wk) {
    ...
    case 3:
        if (ps.plw[id_w].wu.routine_no[3] >= 3) {
            wk->wu.routine_no[3]++;
...
void don_appear_check(PLW* wk) {
    ...
    if (ps.plw[id_w].player_number == 7) {
...
void Appear_36000(PLW* wk) {
    ...
    case 1:
        ...
        if (ps.plw[id_w].wu.cmwk[0] == 3) {
            wk->wu.routine_no[3]++;
            set_char_move_init(&wk->wu, 9, 0x11);
            app_counter[wk->wu.id] = 0x10;
            wk->wu.next_z = ps.plw[id_w].wu.my_priority;
        }
