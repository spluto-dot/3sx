...
void stngauge_control(u8 pl) {
    if (!sdat[pl].proccess_dead) {
        if (ps.plw[pl].dead_flag) {
            sdat[pl].proccess_dead = 1;
            sdat[pl].cstn = 0;
            return;
        }

        if (((ps.plw[pl].wu.routine_no[1] == 1) && (ps.plw[pl].wu.routine_no[2] == 0x19) &&
             (ps.plw[pl].wu.routine_no[3] != 0)) ||
            (ps.plw[pl].py->flag == 1)) {
            sdat[pl].sflag = 1;
        ...
        if (sdat[pl].osflag == 1) {
            ...
            sdat[pl].g_or_s = 0;
            sdat[pl].stimer = 2;
            sdat[pl].cstn = ps.plw[pl].py->now.quantity.h;
            sdat[pl].osflag = sdat[pl].sflag;
        ...
        if (sdat[pl].cstn != ps.plw[pl].py->now.quantity.h) {
            sdat[pl].cstn = ps.plw[pl].py->now.quantity.h;
        }
