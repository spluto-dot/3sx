...
void combo_cont_init() {
    ...
    for (i = 0; i < 2; i++) {
        ...
        cst_read[i] = 0;
        cst_write[i] = 0;
        SDL_zero(ps.plw[i].combo_type);
        SDL_zero(ps.plw[i].remake_power);
...
void combo_control(s8 PL) {
    ...
    if (reversal_check(PL) == 0) {
        ...
        if (!paring_check(PL) && ps.plw[PL].combo_type.total) {
            if (first_attack == 0) {
                first_attack = ps.plw[PL].wu.id + 1;
            ...
            if (cmb_flag != 0) {
                return;
            } else if (ps.plw[PL].combo_type.total == 1) {
...
void check_and_set_combo(s8 PL) {
    ...
    hit_num = ps.plw[PL].combo_type.total;
...
void combo_hensuu_clear(s8 PL) {
    SDL_zero(ps.plw[PL].combo_type);
    combo_rp_clear_check(PL);
    SDL_zeroa(calc_hit[PL]);
    SDL_zeroa(score_calc[PL]);
    bonus_pts[PL] = 0;
    ps.plw[PL].combo_type.total = 0;
    hit_num = 0;
    tr_data[PL].total_damage = 0;
}

void combo_rp_clear_check(s8 PL) {
    if (ps.plw[PL].wu.routine_no[1] != 1 || ps.plw[PL].wu.routine_no[2] != 17 || ps.plw[PL].wu.routine_no[3] == 0 ||
        ps.plw[PL].wu.routine_no[3] == 3) {
        SDL_zero(ps.plw[PL].remake_power);
    }
}

void super_arts_finish_check(s8 PL) {
    if (arts_finish_check2(PL) != 0) {
        if ((ps.plw[PL].combo_type.new_dm & 0x3F) < 48) {
            sa_kind = 2;
        } else {
            sa_kind = 3;
        }
...
void super_arts_last_check(s8 PL) {
    if ((ps.plw[PL].combo_type.new_dm & 0x3F) >= 0x20) {
...
void first_attack_pts_check(s8 PL) {
    if (first_attack - 1 == ps.plw[PL].wu.id) {
        first_attack = 3;
        bonus_pts[PL] += 2;
    }
}
...
void reversal_continue_check(s8 PL) {
    if (ps.plw[PL].wu.routine_no[1] != 4) {
        rever_attack[PL] = 0;
    } else {
        return;
    }
...
void hit_combo_check(s8 PL) {
    s32* sa_ptr = (s32*)ps.plw[PL].combo_type.kind_of[4][0];
    s8 lpx;
...
s32 arts_finish_check2(u8 PL) {
    if (Conclusion_Flag && Conclusion_Type == 0 && Loser_id == PL && (ps.plw[PL].combo_type.new_dm & 0x3F) >= 32) {
        return 1;
    }
...
u32 SCORE_CALCULATION(s8 PL) {
    ...
    k_ptr = ps.plw[PL].combo_type.kind_of[0][0];
    c_ptr = &calc_hit[PL][1];
    s_ptr = score_calc[PL];
...
void combo_window_push(s8 PL, s8 KIND) {
    ...
    if (cmb_stock[PL] == 5) {
        ...
        if (Mode_Type == MODE_VERSUS) {
            Score_Sub();
        } else if (ps.plw[PLS].wu.operator) {
            Score_Sub();
        }
