...
void effe8_zanzou_process(WORK_Other* ewk, PLW* mwk) {
    if (ewk->wu.old_rno[5]) {
        if (ewk->wu.type == 0) {
            ...
            ewk->wu.position_x = ps.zanzou_table[ewk->master_id][ewk->wu.type - 1].pos_x;
            ewk->wu.position_y = ps.zanzou_table[ewk->master_id][ewk->wu.type - 1].pos_y;
        }
    } else {
        ewk->wu.position_x = ps.zanzou_table[ewk->master_id][ewk->wu.type].pos_x;
        ewk->wu.position_y = ps.zanzou_table[ewk->master_id][ewk->wu.type].pos_y;
    }
    ...
    if (ewk->wu.old_rno[3] == 0) {
        ...
        ewk->wu.cg_number = ps.zanzou_table[ewk->master_id][ewk->wu.type].cg_num;
        ewk->wu.rl_flag = ps.zanzou_table[ewk->master_id][ewk->wu.type].flip;
        ewk->wu.cg_flip = ps.zanzou_table[ewk->master_id][ewk->wu.type].cg_flp;
    }
