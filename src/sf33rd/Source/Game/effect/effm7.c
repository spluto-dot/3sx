...
void effm7_move(WORK_Other* ewk) {
    ...
    case 0:
        ...
        ewk->wu.rl_flag ^= ps.plw[id_w].wu.rl_flag;

        if (ps.plw[id_w].wu.rl_flag) {
            ewk->wu.xyz[0].disp.pos = ps.plw[id_w].wu.xyz[0].disp.pos - ewk->wu.xyz[0].disp.pos;
        } else {
            ewk->wu.xyz[0].disp.pos = ps.plw[id_w].wu.xyz[0].disp.pos + ewk->wu.xyz[0].disp.pos;
        }
...
s32 effect_M7_init(PLW* oya) {
    ...
    for (i = 0; i < 6; i++) {
        ...
        ewk->wu.xyz[1].cal = ps.plw[em_id].wu.xyz[1].cal;
        ewk->wu.xyz[1].disp.pos += *(s16*)data_ptr++;
        ewk->wu.position_z = ps.plw[em_id].wu.my_priority;
