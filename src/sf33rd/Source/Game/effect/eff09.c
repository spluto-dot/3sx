    ...
    case 1:
        if (!EXE_flag && !Game_pause) {
            ...
            if (ewk->master_id) {
                sw_work = p2sw_0;
                hit_pl = &ps.plw[0];
                pl_hit_ptr = &ps.plw[0].wu.h_bod->body_dm[0][0];
            } else {
                sw_work = p1sw_0;
                hit_pl = &ps.plw[1];
                pl_hit_ptr = &ps.plw[1].wu.h_bod->body_dm[0][0];
            }
            ...
            if (hit_check_subroutine(&hit_pl->wu, &ewk->wu, pl_hit_ptr, sean_ball_hit)) {
                ...
                add_super_arts_gauge(
                    ps.plw[ewk->master_id].sa, ps.plw[ewk->master_id].wu.id, 1, ps.plw[ewk->master_id].metamorphose);
                break;
            }
...
void sean_ball_move(WORK_Other* ewk, u16 sw_work) {
    if (!ps.plw[ewk->master_id].wu.operator) {
        return;
    }
...
void eff09_21000(WORK_Other* ewk) {
    ...
    case 0:
        ...
        if (ewk->wu.type == 29) {
            ...
            if (ps.plw[Winner_id].wu.rl_flag) {
                arrive_x = ps.plw[Winner_id].wu.xyz[0].disp.pos + 28;
            } else {
                arrive_x = ps.plw[Winner_id].wu.xyz[0].disp.pos - 28;
            }

            arrive_y = ps.plw[Winner_id].wu.xyz[1].disp.pos + 162;
        }
